...
bool filename_is_okey(boost::filesystem::path &p) {
    ...
    //месяц
    if (filename[21] > 1 || (filename[21] == 1 && filename[22] > 2))return false;
    //число
    if (filename[23] > 3 || (filename[23] == 3 && filename[24] > 1))return false;

    return !(p.extension() != ".txt");

}

//std::vector<std::unordered_map< std::string, boost::filesystem::path>> bypass(boost::filesystem::path &p) {
std::vector<finance> bypass(boost::filesystem::path &p) {
    if (!exists(p)) throw std::logic_error("path doesn't exist");

//    std::vector<std::unordered_map<std::string, boost::filesystem::path>> files, temp;
//    std::unordered_map<std::string, boost::filesystem::path> map;
    ...
    for (const boost::filesystem::directory_entry &x : boost::filesystem::directory_iterator{p}) {
        if (is_regular_file(x)) {
            ...
            if (filename_is_okey(temp_path)) {
                std::cout << temp_path.parent_path() << "   " << temp_path.filename() << std::endl;
...
//            vec.erase(vec.begin() + i);
    ...
    std::cout << std::endl << "broker:  " << newest.broker() << "  account: " << newest.account() << "  files:  "
              << files << "  last date:  " << newest.date();
    lasts(vec);
}
